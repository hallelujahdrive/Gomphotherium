...
namespace Valastodon {
	...
	public class Status : Valastodon.Object {
		...
		private string? _in_reply_to_id;  // (Nullable) null or the ID of the status it replies to
		private string? _in_reply_to_account_id;  // (Nullable) null or the ID of the account it replies to
		private Status _reblog; // (Nullable) null or the reblogged Status
		...
		private bool _reblogged; // (Nullable) Whether the authenticated user has reblogged the status
		private bool _favourited;  // (Nullable) Whether the authenticated user has favourited the status
		...
		private string _language; // The detected language for the status, if detected
		private bool _pinned; // Whether this is the pinned status for the account that posted it
		...
		internal Status (Json.Object json_obj) {
			...
			json_obj.foreach_member ((obj, mem, node) => {
				...
				switch (mem) {
					...
					break;
					case "language" : _language = node.get_string ();
					break;
					case "pinned" : _pinned = node.get_boolean ();
					break;
				}
